...
void Nokia5110_Init(void){
  ...
  GPIO_PORTA_AFSEL_R &= (unsigned long)(~0xC0);          // disable alt funct on PA6,7
  ...
  GPIO_PORTA_AMSEL_R &= (unsigned long)(~0xEC);          // disable analog functionality on PA2,3,5,6,7
  SSI0_CR1_R &= (unsigned long)(~SSI_CR1_SSE);           // disable SSI
  SSI0_CR1_R &= (unsigned long)(~SSI_CR1_MS);            // master mode
                                        // configure for system clock/PLL baud clock source
  SSI0_CC_R = (SSI0_CC_R&(unsigned long)(~SSI_CC_CS_M))+SSI_CC_CS_SYSPLL;
                                        // clock divider for 3.125 MHz SSIClk (50 MHz PIOSC/16)
  SSI0_CPSR_R = (SSI0_CPSR_R&(unsigned long)(~SSI_CPSR_CPSDVSR_M))+16;
  SSI0_CR0_R &= (unsigned long)(~(SSI_CR0_SCR_M |       // SCR = 0 (3.125 Mbps data rate)
                  SSI_CR0_SPH |         // SPH = 0
                  SSI_CR0_SPO));         // SPO = 0
                                        // FRF = Freescale format
  SSI0_CR0_R = (SSI0_CR0_R&~(unsigned long)(~SSI_CR0_FRF_M))+SSI_CR0_FRF_MOTO;
                                        // DSS = 8-bit data
  SSI0_CR0_R = (SSI0_CR0_R&(unsigned long)(~SSI_CR0_DSS_M))+SSI_CR0_DSS_8;
  ...
  lcdwrite(COMMAND, 0xBF);              // set temp coefficient
...
void Nokia5110_OutUDec(unsigned short n){
  if(n < 10){
    Nokia5110_OutString("    ");
    Nokia5110_OutChar((unsigned char)(n+'0')); /* n is between 0 and 9 */
  } else if(n<100){
    Nokia5110_OutString("   ");
    Nokia5110_OutChar((unsigned char)(n/10+'0')); /* tens digit */
    Nokia5110_OutChar((unsigned char)(n%10+'0')); /* ones digit */
  } else if(n<1000){
    Nokia5110_OutString("  ");
    Nokia5110_OutChar((unsigned char)(n/100+'0')); /* hundreds digit */
    n = n%100;
    Nokia5110_OutChar((unsigned char)(n/10+'0')); /* tens digit */
    Nokia5110_OutChar((unsigned char)(n%10+'0')); /* ones digit */
  }
    ...
    Nokia5110_OutChar(' ');
    Nokia5110_OutChar((unsigned char)(n/1000+'0')); /* thousands digit */
    n = n%1000;
    Nokia5110_OutChar((unsigned char)(n/100+'0')); /* hundreds digit */
    n = n%100;
    Nokia5110_OutChar((unsigned char)(n/10+'0')); /* tens digit */
    ...
    Nokia5110_OutChar((unsigned char)(n/10000+'0')); /* ten-thousands digit */
    n = n%10000;
    Nokia5110_OutChar((unsigned char)(n/1000+'0')); /* thousands digit */
    n = n%1000;
    Nokia5110_OutChar((unsigned char)(n/100+'0')); /* hundreds digit */
